...
def get_position_row(symbol: str, side: str, entry: float, current: float, size: float, margin: float, pnl: float, stop_price: float):
    ...
    details = Text.assemble(
        ("Entry: ", C_DIM), (f"{entry:.4g}", C_WHITE),
        ("  Price: ", C_DIM), (f"{current:.4g}", C_WHITE),
        ("  PnL: ", C_DIM), (f"${pnl:+.2f}", f"bold {pnl_style}")
    )
    ...
    for t in (ticker_data or [])[:10]:
        try:
            ...
            dbg_log(
                ...
                data={"exc_type": type(e).__name__, "exc": repr(e), "ticker_keys": list(t.keys()) if isinstance(t, dict) else None},
            )
            #endregion
    
    return Panel(table, title=f"[bold {C_CYAN}]MARKET PULSE[/bold {C_CYAN}]", border_style=C_CYAN)
...
def get_log_view(logs: List[str]):
    ...
    for line in logs[-20:]:
        ...
        style = C_WHITE
        if "ERROR" in line or "FAIL" in line: style = C_RED
        elif "WARNING" in line: style = C_YELLOW
        elif "SUCCESS" in line or "VERIFIED" in line: style = C_GREEN
        elif "INFO" in line: style = C_CYAN
        
        log_text.append(line + "\n", style=style)
    
    return Panel(log_text, title=f"[bold {C_CYAN}]SYSTEM AUDIT[/bold {C_CYAN}]", border_style=C_DIM)
