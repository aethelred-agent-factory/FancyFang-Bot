...
p_bot_storage = StorageManager(
    Path(SCRIPT_DIR).parent / "data" / "state" / "fancybot.db"
)
if _SA_OK:
...
def make_entity_request(
    ...
    if not ENABLE_ENTITY:
        return None
    return pc.make_entity_request(
        entity_name, method=method, data=data, entity_id=entity_id
    )
...
MIN_PREDICTIVE_SCORE_HTF_BYPASS = float(
    os.getenv("BOT_MIN_PREDICTIVE_SCORE_HTF", "0.1")
)
MIN_PREDICTIVE_SCORE_LOW_LIQ = float(
    os.getenv("BOT_MIN_PREDICTIVE_SCORE_LOW_LIQ", "0.1")
)
...
_local_stop_states: Dict[str, dict] = (
    {}
)  # symbol -> {stop_price, high_water, low_water, entry_time, entry_score, direction}
...
BLACKLIST_DURATION_SECONDS = int(
    os.getenv("BOT_BLACKLIST_SECONDS", "1800")
)  # 30 min (fallback)
...
def load_blacklist() -> None:
    ...
    with _blacklist_lock:
        ...
        for symbol, expiry_ts in loaded_data.items():
            ...
            try:
                p_bot_storage.add_to_blacklist(
                    symbol, reason="migrated", expires_at=expires_dt
                )
                migrated += 1
            except Exception as db_err:
                logger.error(
                    f"Failed to migrate blacklist symbol {symbol} to DB: {db_err}"
                )
...
BASE_COOLDOWN_LOSS_S = int(
    os.getenv("BASE_COOLDOWN_LOSS_S", "1800")
)  # 30 mins base on loss
PNL_COOLDOWN_MULTIPLIER = int(
    os.getenv("PNL_COOLDOWN_MULTIPLIER", "72")
)  # 72s per dollar lost (e.g., -$25 loss adds 30 mins)
ENTROPY_COOLDOWN_REDUCTION_F = int(
    os.getenv("ENTROPY_COOLDOWN_REDUCTION_F", "120")
)  # 120s reduction per entropy point
...
ACCOUNT_TRAIL_PCT = float(
    os.getenv("BOT_ACCOUNT_TRAIL_PCT", "0.05")
)  # 5% trail on peak equity
...
def _check_stops_live(symbol):
    ...
    if current is None:  # Price not available from WS
        if not _ws_connected:  # WS is explicitly disconnected
            logger.warning(
                f"WS disconnected. Attempting REST API fallback for {symbol} in _check_stops_live."
            )
            ...
            if current:
                ...
                logger.info(
                    f"Successfully obtained REST API price for {symbol} in _check_stops_live."
                )
            else:
                logger.warning(
                    f"Failed to get price for {symbol} from REST API. Cannot check stop."
                )
                return
        else:
            logger.debug(
                f"WS connected but no live price for {symbol}. Waiting for WS update."
            )
            return
...
def _check_account_trail():
    ...
    with _prices_lock:
        missing_symbols = [
            ...
            if _live_prices.get(pos["symbol"]) is None
        ]
    ...
    if missing_symbols and not _ws_connected:
        for symbol in missing_symbols:
            logger.warning(
                f"WS disconnected. Attempting REST API fallback for {symbol} in _check_account_trail."
            )
            ...
            if price:
                with _prices_lock:
                    _live_prices[symbol] = price
                logger.info(
                    f"Successfully obtained REST API price for {symbol} in _check_account_trail."
                )
...
def _ws_on_close(ws, close_status_code, close_msg):
    ...
    _ws_connected = False
    logger.warning(
        f"WS Connection Closed: Status Code={close_status_code}, Message={close_msg}"
    )
...
def _ws_run_loop():
    ...
    while _running:
        try:
            logger.info(
                f"Attempting WS connection... (current _ws_connected: {_ws_connected}, next retry in {reconnect_delay}s)"
            )
            ...
            threading.Thread(
                target=_ws_heartbeat, args=(_ws_app, stop_event), daemon=True
            ).start()
            ...
            logger.error(
                f"WS run loop error: {e}. Reconnecting in {reconnect_delay}s..."
            )
            ...
            if new_balance is not None:
                with _cache_lock:
                    ...
                    if closed:
                        ...
                        for sym in closed:
                            old_p = next(
                                (p for p in _cached_positions if p["symbol"] == sym),
                                None,
                            )
                            ...
                            entry_price = (
                                old_p.get("entry", 0)
                                if old_p
                                else (h_entry.get("price", 0) if h_entry else 0)
                            )
                            qty_str = (
                                str(old_p.get("size", 0))
                                if old_p
                                else (str(h_entry.get("qty", 0)) if h_entry else "0")
                            )
                            score = local_state.get("entry_predictive_score", 0.0) or (
                                h_entry.get("predictive_score", 0.0) if h_entry else 0.0
                            )
                            entry_time = local_state.get("entry_time") or (
                                datetime.datetime.fromisoformat(h_entry["timestamp"])
                                if h_entry
                            ...
                            direction = local_state.get(
                                "direction",
                                (
                                    "LONG"
                                    if (old_p and old_p["side"] == "Buy")
                                    else (
                                        h_entry.get("direction")
                                        if h_entry
                                        else "Unknown"
                                    )
                                ),
                            ...
                            hold_secs = (
                                (now_utc - entry_time).total_seconds()
                                if entry_time
                                else 0
                            )
                            ...
                            dur_str = (
                                ...
                                if h_hour > 0
                                    ...
                                    f"{h_min}m {h_sec}s" if h_min > 0 else f"{h_sec}s"
                                )
                            )
                            ...
                            with _equity_lock:
                                with _cache_lock:
                                    current_equity = _cached_balance + sum(
                                        p.get("pnl", 0.0) for p in _cached_positions
                                    )
                            ...
                            if _DD_OK:
                                try:
                                    drawdown_guard.record_pnl(
                                        float(realized_pnl),
                                        (
                                            new_balance
                                            if new_balance is not None
                                            else _cached_balance
                                        ),
                                    )
                                except Exception as e:
                            ...
                            if _RM_OK:
                                try:
                                    ...
                                    logger.warning(
                                        f"[RM] record_trade_result failed: {e}"
                                    )
                ...
                if _account_high_water > 0:
                    drawdown = (
                        (_account_high_water - equity) / _account_high_water * 100
                    )
                ...
                make_entity_request(
                    ...
                    data={
                        ...
                        "max_positions_allowed": get_dynamic_max_positions(
...
def _subscribe_symbol(symbol):
    def _do_sub():
        ...
        try:
            ...
            logger.error(
                f"Subscription thread failed for {symbol}: {e}\n{traceback.format_exc()}"
            )
...
def _get_session_chart() -> Optional[str]:
    ...
    try:
        ...
        plt.title(
            f"Session Equity Curve ({datetime.datetime.now().strftime('%Y-%m-%d %H:%M')})"
        )
    ...
    try:
        ...
        if not top:
            ...
            for r in top:
                ...
                lines.append(
                    f"{arrow} `{r['inst_id']}` | Score: `{r['score']}` | Price: `{r['price']:.5g}`"
                )
    ...
    with term.fullscreen(), term.hidden_cursor():
        while _display_thread_running:
            ...
            closed_trades = [
                t
                for t in all_trades
                if t.get("status") == "closed" or "pnl" in t and t["pnl"] != 0
            ]
            ...
            for i, line in enumerate(banner_lines):
                print(
                    term.move_xy(2, i + 1)
                    + ui.gradient_text(line, (0, 255, 255), (255, 0, 255))
                )

            header_y = len(banner_lines) + 1
            print(term.move_xy(2, header_y) + ui.hr_double(Fore.MAGENTA))
            print(
                term.move_xy(2, header_y + 1)
                + term.bold_white(
                    f" {Fore.MAGENTA}{pulse}{Style.RESET_ALL} LIVE PRODUCTION DASHBOARD | {curr_time} UTC"
                )
            )
            ...
            for pos in positions:
                ...
                with _prices_lock:
                    ...
                    if now:
                        upnl = (
                            (now - pos["entry"]) * pos["size"]
                            if pos["side"] == "Buy"
                            else (pos["entry"] - now) * pos["size"]
                        )
                        total_upnl += upnl
            ...
            equity = balance + total_upnl
            halt_status = (
                f" {Fore.RED}[HALTED]{Style.RESET_ALL}"
            ...
            with _blacklist_lock:
                bl_active = {
                    s: exp for s, exp in SYMBOL_BLACKLIST.items() if time.time() < exp
                }
                if bl_active:
                    bl_str = ", ".join(
                        f"{s}({int((e - time.time()) / 60)}m)" for s, e in bl_active.items()
                    )
                    summary_lines.append(
                        f" {Fore.YELLOW}🚫 COOLDOWN: {bl_str[:left_w-15]}{Style.RESET_ALL}"
                    )
            ...
            draw_panel(
                ...
                y,
                ui.glow_panel(
                    "SYSTEM CORE", summary_lines, color_rgb=(0, 255, 255), width=left_w
                ),
            )
            ...
            if not positions:
                draw_panel(
                    ...
                    ui.modern_panel(
                        "ACTIVE POSITIONS",
                        [Fore.WHITE + " (Awaiting entry signals...)"],
                        width=left_w,
                    ),
                )
                y += 4
                ...
                for pos in positions[:max_show]:
                    ...
                    if not now:
                        draw_panel(
                            ...
                            y,
                            ui.modern_panel(
                                sym, [Fore.WHITE + "Waiting for price..."], width=left_w
                            ),
                        )
                        y += 4
                        continue
                    ...
                    dir_badge = (
                        f"{Fore.GREEN}▲ LONG{Style.RESET_ALL}"
                        if side == "Buy"
                        else f"{Fore.RED}▼ SHORT{Style.RESET_ALL}"
                    )
                    ...
                    pos_header = (
                        f"{dir_badge} {term.bold_white(sym)} {pnl_str} {dur_str}"
                    )
                    pos_info = [
                    ...
                    draw_panel(
                        ...
                        y,
                        ui.glow_panel(
                            pos_header, pos_info, width=left_w, color_rgb=(255, 0, 255)
                        ),
                    )
                    y += len(pos_info) + 2
                ...
                if len(positions) > max_show:
                    print(
                        term.move_xy(2, y)
                        + term.italic_white(
                            f"  ... and {len(positions) - max_show} more positions hidden"
                        )
                    )
                    y += 2
            ...
            draw_panel(
                ...
                log_y,
                ui.modern_panel(
                    "SYSTEM LOGS", log_lines, width=left_w, color=Fore.WHITE
                ),
            )
            ...
            if closed_trades:
                ...
                hist_lines.append(
                    f"{Fore.WHITE}Wins: {Fore.GREEN}{wins}{Style.RESET_ALL} | Loss: {Fore.RED}{len(closed_trades)-wins}{Style.RESET_ALL} | WR: {wr:.1f}%"
                )
                hist_lines.append(
                    f"Total PnL: {ui.pnl_color(tot_pnl)}{tot_pnl:+.2f} USDT{Style.RESET_ALL}"
                )
            ...
            for t in reversed(closed_trades[-15:]):
                ...
                hist_lines.append(
                    f" {ts} {sym:<6} {ui.pnl_color(p)}{p:+.2f}{Style.RESET_ALL}"
                )
            ...
            draw_panel(
                ...
                hist_y,
                ui.modern_panel(
                    "TRADE HISTORY", hist_lines, width=right_w, color=Fore.CYAN
                ),
            )
...
def _load_instruments():
    ...
    if not data or data.get("code") != 0:
        logger.warning(
            "Could not load instrument data — will use fallback qty rounding"
        )
    ...
    for prod in data.get("data", {}).get("perpProductsV2") or []:
        ...
        step_str = prod.get("qtyStepSize") or prod.get("qtyStepSizeRq") or "0.001"
        try:
            step = float(step_str)
        except Exception:
            step = 0.001
...
def set_leverage(symbol: str, leverage: int, pos_side: Optional[str] = None) -> bool:
    ...
    if isinstance(result, dict) and result.get("code") == 20003:
        ...
        logger.warning(
            f"TE_ERR_INVALID_LEVERAGE for {symbol} at {leverage}x. Retrying with {fallback_leverage}x."
        )

        result_fallback = _put(
            "/g-positions/leverage",
            params={
                "symbol": symbol,
        ...
        if result_fallback and result_fallback.get("code") == 0:
            logger.info(
                f"Leverage successfully set to {fallback_leverage}x for {symbol} after fallback."
            )
            return True
        else:
            logger.error(
                f"Fallback leverage to {fallback_leverage}x also failed for {symbol}: {result_fallback}"
            )
            return False
...
def _switch_pos_mode(symbol: str, target_mode: str) -> bool:
    ...
    params = {"symbol": symbol, "targetPosMode": target_mode}
    ...
    if isinstance(result, dict) and result.get("code") is not None:
        logger.error(
            f"Failed to set position mode for {symbol} to {target_mode}: {result.get('msg')} (Code {result.get('code')})"
        )
    elif result is not None:
        logger.error(
            "Failed to set position mode for %s to %s: Unexpected response format: %s",
            symbol,
            target_mode,
            result,
        )
        ...
        logger.error(
            "Failed to set position mode for %s to %s: No response from API",
            symbol,
            target_mode,
        )
    return False
...
def place_trailing_stop(
    ...
    if result:
        logger.info(
            f"[TRAIL STOP] {symbol} response: code={result.get('code')} data={json.dumps(result.get('data', {}))[:200]}"
        )
    else:
...
def cancel_order_by_client_id(symbol: str, client_order_id: str) -> bool:
    ...
    result = _delete(
        "/g-orders/cancel", params={"symbol": symbol, "clOrdID": client_order_id}
    )
    if result and result.get("code") == 0:
        ...
        if log_file_jsonl.exists():
            ...
            try:
                ...
                if isinstance(old, list):
                    ...
                    logger.info(
                        f"Migrated {len(old)} legacy trades to {log_file_jsonl}"
                    )
            except Exception as e:
...
def verify_candidate(
    symbol: str, direction: str, original_score: int, wait_seconds: int = 30
) -> Optional[dict]:
    """
    ...
    for i in range(steps):
        ...
        if not ticker:
            tui_log(
                f"FAIL: {symbol} ticker not found during verification.",
                event_type="FAIL",
            )
            return None
        ...
        if direction == "LONG" and price_change < -0.8:
            tui_log(
                f"FAIL: {symbol} dropping during verify: {price_change:+.2f}%",
                event_type="FAIL",
            )
            return None
        elif direction == "SHORT" and price_change > 0.8:
            tui_log(
                f"FAIL: {symbol} pumping during verify: {price_change:+.2f}%",
                event_type="FAIL",
            )
            return None
        ...
        fresh_result = scanner.analyse(
            ticker, cfg, enable_ai=False, enable_entity=False
        )
        if not fresh_result:
            tui_log(
                f"FAIL: {symbol} no longer qualifies at step {i+1}", event_type="FAIL"
            )
            return None
        ...
        if not spread_ok:
            tui_log(
                f"FAIL: {symbol} spread too wide: {spread_reason}", event_type="FAIL"
            )
            return None
        ...
        if fresh_score < original_score * 0.90:
            tui_log(
                f"FAIL: {symbol} score degraded: {original_score}->{fresh_score}",
                event_type="FAIL",
            )
            return None
    ...
    if abs(final_change) > 2.0:
        tui_log(
            f"FAIL: {symbol} overextended ({final_change:+.2f}%) during wait.",
            event_type="FAIL",
        )
        return None
...
def execute_setup(result: dict, direction: str, dry_run: bool = False) -> bool:
    ...
    if is_blacklisted(symbol):
        ...
        tui_log(
            f"SKIP: {symbol} is on cooldown ({remaining:.0f}m remaining)",
            event_type="SKIP",
        )
        return False
    ...
    if _CM_OK:
        with _cache_lock:
            blocked, reason = corr_mgr.correlation_mgr.should_block_entry(
                symbol, direction, _cached_positions
            )
        if blocked:
    ...
    is_htf_aligned = any(
        "HTF Alignment" in s for s in signals
    )  # HTF alignment might still be part of signals from prediction engine
    ...
    effective_min_predictive = (
        MIN_PREDICTIVE_SCORE_LOW_LIQ
        if is_low_liq
        else (
    ...
    if predictive_score < effective_min_predictive:
        gate_reason = (
            "low-liq" if is_low_liq else ("htf-ok" if is_htf_aligned else "no-HTF")
        )
        tui_log(
            f"SKIP: {symbol} predictive score {predictive_score:.2f} < effective min {effective_min_predictive:.2f} ({gate_reason})",
            event_type="SKIP",
        )
        return False
    ...
    if _RM_OK:
        try:
            ...
            risk_usd, _ = risk_mgr.compute_dynamic_risk(
                ...
                signal_strength=min(
                    predictive_score, 2.0
                ),  # Cap predictive score for signal strength input
                stop_distance=stop_distance,
                open_positions=_cached_positions,
                available_liquidity=result.get("depth"),
            )
            # reject_trade check
            reject, reject_reason = risk_mgr.should_reject_trade(
                risk_usd, _cached_balance, _cached_positions
            )
            if reject:
                tui_log(
                    f"RISK MGR REJECT: {symbol} — {reject_reason}", event_type="SKIP"
                )
                return False
            active_margin = max(risk_usd, 1.0)  # floor at $1 to avoid dust orders
            tui_log(
                f"RISK MGR: {symbol} dynamic margin=${active_margin:.2f} (predictive_score={predictive_score:.2f}, bal={_cached_balance:.2f})",
                event_type="INFO",
            )
        except Exception as e:
            logger.warning(
                f"[RISK MGR] compute_dynamic_risk failed for {symbol}: {e} — using static margin"
            )
    ...
    pos_side = (
        "Merged"
        if POSITION_MODE == "OneWay"
        else ("Long" if direction == "LONG" else "Short")
    )

    tui_log(
        f"EXECUTING {arrow} {symbol} | Predictive Score: {predictive_score:.2f} | Price: {price:.4g} | Qty: {qty_str} | Margin: ${active_margin} | Lev: {active_leverage}x",
        event_type="EXEC",
    )
    ...
    make_entity_request(
        ...
        data={
            ...
            "timestamp": datetime.datetime.now(
                datetime.timezone.utc
            ).isoformat(),  # REF: Tier 3: Temporal Inconsistency
            "parameters": {
            ...
            "market_context": {"htf_aligned": is_htf_aligned, "signals": signals[:10]},
        },
    )
    ...
    if dry_run:
        ...
        make_entity_request(
            "trade", method="PUT", entity_id=trade_id, data={"status": "DRY_RUN"}
        )
        return True
    ...
    if not mode_ok:
        print(
            Fore.RED
            + f" [ERROR] Failed to set position mode for {symbol} to {target_mode}. Cannot proceed."
        )
        return False
    ...
    if not lev_ok:
        print(
            Fore.YELLOW
            + " [WARN] Leverage set returned non-zero — proceeding anyway (check logs for details)"
        )
    ...
    if not order_resp:
        tui_log(
            f"ERROR: No response from order endpoint for {symbol}", event_type="ERROR"
        )
        make_entity_request(
            "trade",
            method="PUT",
            entity_id=trade_id,
            data={"status": "ORDER_FAILED", "outcome": "NO_RESPONSE"},
        )
        return False
    ...
    if code != 0:
        biz_err = (
            order_resp.get("data", {}).get("bizError")
            if isinstance(order_resp.get("data"), dict)
            else None
        )
        tui_log(
            f"ERROR: Order failed for {symbol}: code={code} bizError={biz_err}",
            event_type="ERROR",
        )
        make_entity_request(
            "trade",
            method="PUT",
            entity_id=trade_id,
            data={"status": "ORDER_FAILED", "outcome": f"CODE_{code}"},
        )
        return False
    ...
    tui_log(
        f"Entry order accepted | {symbol} | orderID: {order_id} | status: {exec_status} | Price: {avg_price:.6g}",
        event_type="ENTRY",
    )
    ...
    for i in range(max_ts_retries):
        tui_log(
            f"Placing trailing stop ({active_trail_pct*100:.1f}%, {close_side}) for {symbol} attempt {i+1}/{max_ts_retries}...",
            event_type="STOP",
        )
        ts_resp = place_trailing_stop(
            symbol, close_side, qty_str, price, active_trail_pct
        )

        if ts_resp and ts_resp.get("code") == 0:
            ts_id = ts_resp.get("data", {}).get("orderID", "?")
            tui_log(
                f"Trailing stop placed | {symbol} | orderID: {ts_id}", event_type="STOP"
            )
            ts_ok = True
            ...
            make_entity_request(
                ...
                method="PUT",
                entity_id=trade_id,
                data={"status": "MONITORING", "exit": 0},
            )
            break  # Exit retry loop on success
        else:
            ts_biz = (
                ts_resp.get("data", {}).get("bizError")
                if isinstance(ts_resp.get("data"), dict)
                else None
            )
            logger.warning(
                f"Trailing stop failed (attempt {i+1}): code={ts_resp.get('code')} bizError={ts_biz}. Retrying..."
            )
            time.sleep(2)  # Wait before retrying

    if not ts_ok:
        tui_log(
            f"CRITICAL ERROR: Failed to place trailing stop for {symbol} after {max_ts_retries} attempts.",
            event_type="CRITICAL",
        )
        tui_log(
            f"Cancelling entry order {order_id} (clOrdID: {entry_clord_id}) to prevent unprotected position!",
            event_type="CRITICAL",
        )
        make_entity_request(
            "trade",
            ...
            entity_id=trade_id,
            data={
                "status": "STOP_FAILED_AND_CANCELLED",
                "outcome": "TRAIL_STOP_FAILURE",
            },
        )

        # Note: We didn't append to _cached_positions yet, so no need to filter it out.

        if cancel_order_by_client_id(
            symbol, entry_clord_id
        ):  # entry_clord_id is the client order ID for the entry
            tui_log(
                f"Entry order {order_id} cancelled successfully for {symbol}.",
                event_type="CRITICAL",
            )
            # REF: Tier 3: Temporal Inconsistency
            log_trade(
            ...
            tui_log(
                f"CRITICAL ERROR: Failed to cancel entry order {order_id}. MANUAL INTERVENTION REQUIRED for {symbol}!",
                event_type="CRITICAL",
            )
            # Send an emergency telegram message
            send_telegram_message(
                f"🚨 *URGENT:* Failed to place trailing stop AND failed to cancel entry for {symbol}. Manual intervention needed!"
            )
...
def _scan_one(module, direction: str, cfg: dict, args, on_result=None) -> List[dict]:
    ...
    if symbols_to_scan:
        filtered_tickers = [
            ticker for ticker in tickers if ticker.get("symbol") in symbols_to_scan
        ]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = [
            executor.submit(
                module.analyse, t, cfg, not args.no_ai, not args.no_entity, None
            )
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                if scan_res:
                    scan_res["scan_timestamp"] = datetime.datetime.now(
                        datetime.timezone.utc
                    )
                    ...
                    if on_result:
                        ...
                        def _result_wrapper(res, dr):
                            try:
                                ...
                                logger.error(
                                    f"On-result callback crashed: {error}\n{traceback.format_exc()}"
                                )

                        threading.Thread(
                            target=_result_wrapper,
                            args=(scan_res, direction),
                            daemon=True,
                        ).start()
            except Exception as error:
...
def pick_candidates(
    ...
    for scan_res in long_results:
        symbol_scores.setdefault(scan_res["inst_id"], {"LONG": 0, "SHORT": 0})[
            "LONG"
        ] = scan_res["score"]
...
def bot_loop(args):
    ...
    _account_high_water = _cached_balance + sum(
        [p.get("pnl", 0.0) for p in _cached_positions]
    )
        ...
        if p["symbol"] not in _local_stop_states:
            ...
            h_entry = next(
                (
                    h
                    for h in reversed(history)
                    if h.get("symbol") == p["symbol"] and h.get("status") == "entered"
                ),
                None,
            )
            entry_time = datetime.datetime.now(datetime.timezone.utc)
            ...
            if h_entry:
                try:
                    ...
                    logger.warning(
                        f"Failed to parse entry timestamp for {p['symbol']} from history: {e}"
                    )
    ...
    if not _display_thread_running:
        ...
        def _display_wrapper():
            ...
            try:
                ...
                logger.error(
                    f"Display thread crashed: {error}\n{traceback.format_exc()}"
                )
        ...
        def _cache_wrapper():
            ...
            try:
                ...
                logger.error(
                    f"Cache refresher thread crashed: {error}\n{traceback.format_exc()}"
                )
    ...
    if _DD_OK:
        ...
        logger.info(
            f"[DD] Drawdown guard armed — start balance: {_cached_balance:.2f} USDT"
        )
    ...
    while _running:
        try:
            ...
            if pc.is_hour_blocked():
                ...
                tui_log(
                    f"HOUR FILTER: Skipping scan cycle — hour {curr_hour} UTC is blocked.",
                    event_type="SKIP",
                )
                time.sleep(60)
                continue
            ...
            if _EF_OK:
                ...
                if suppressed:
                    tui_log(
                        f"EVENT FILTER: Skipping scan cycle — {reason}",
                        event_type="SKIP",
                    )
                    time.sleep(60)
                    continue
            ...
            if _DD_OK:
                ...
                if not _dd_ok_flag:
                    tui_log(
                        f"[DD] Trading halted by drawdown guard: {_dd_reason}",
                        event_type="HALT",
                    )
                    time.sleep(60)
                    continue
            ...
            if _TG_OK and telegram.is_halted():
                tui_log(
                    "[TG] Trading halted via Telegram /stop command", event_type="HALT"
                )
                time.sleep(30)
                continue
            ...
            with _bot_state.lock:
                ...
                _bot_state.rolling_stats = {
                    ...
                    "losses": _session_losses,
                    "win_pnl": (
                        _session_total_pnl if _session_total_pnl > 0 else 0
                    ),  # Best guess
            ...
            def on_scan_result(scan_res, direction, args=args):
                ...
                intensity = (
                    tracker.get_intensity()
                )  # Check intensity WITHOUT updating it yet
                hawkes_penalty = _get_cluster_threshold_penalty(intensity)

                # Use global _entropy_penalty from last scan to block cascades
                effective_fast_track = (
                    FAST_TRACK_SCORE + hawkes_penalty + _entropy_penalty
                )
                if scan_res["score"] < effective_fast_track:
                    if hawkes_penalty > 0 or _entropy_penalty > 0:
                        tui_log(
                            f"FT THROTTLE: {scan_res['inst_id']} score {scan_res['score']} < dynamic FT threshold {effective_fast_track} (λ={intensity:.2f}, H_pen={_entropy_penalty})",
                            event_type="THROTTLE",
                        )
                    return
                ...
                if result_time:
                    ...
                    if (
                        datetime.datetime.now(datetime.timezone.utc) - result_time
                    ).total_seconds() > RESULT_STALENESS_SECONDS:
                        return

                with _fast_track_lock:
                    with _cache_lock:
                        # Account for positions already open PLUS those currently being verified
                        if len(_cached_positions) + len(
                            _fast_track_opened
                        ) >= get_dynamic_max_positions(_cached_balance):
                            return
                        if scan_res["inst_id"] in {
                            p["symbol"] for p in _cached_positions
                        }:
                            return
                    ...
                    if scan_res["score"] > 130:
                        ...
                        def _fetch_thesis(res, dr):
                            ...
                            def _token_cb(token):
                                if (
                                    not _thesis_log
                                    or res["inst_id"] not in _thesis_log[-1]
                                ):
                ...
                try:
                    tui_log(
                        f"FAST-TRACK: {scan_res['inst_id']} scored {scan_res['score']} — opening immediately (λ={intensity:.2f})",
                        event_type="FAST",
                    )
                    ...
                    verified_result = verify_candidate(
                        scan_res["inst_id"], direction, scan_res["score"]
                    )

                    if verified_result:
                        ok = execute_setup(
                            verified_result, direction, dry_run=args.dry_run
                        )
                        if ok:
            ...
            # ── Scan ─────────────────────────────────────────────────────
            if (
                _cached_balance < LOW_LIQ_MARGIN
            ):  # allow entry even in low-liq mode ($5 min)
            ...
            long_results, short_results = run_scanner_both(
                cfg, args, on_result=on_scan_result
            )
            _display_paused.clear()
            ...
            total_universe = len(
                [
                    ...
                    if float(t.get("turnoverRv", 0)) >= args.min_vol
                ]
            )
            ...
            if _entropy_penalty > ENTROPY_ALERT_LEVEL:
                tui_log(
                    f"ENTROPY DEFLATOR: Raising min_score by +{_entropy_penalty} (Saturation: {n_hits}/{total_universe}, Imbalance: {imbalance:.2f})",
                    event_type="DEFLATOR",
                )
            ...
            if eff_min_score > args.min_score:
                tui_log(
                    f"ADAPTIVE FILTER: Effective Min Score: {eff_min_score} (Penalty: +{_entropy_penalty})",
                    event_type="ADAPTIVE",
                )
            ...
            with _bot_state.lock:
                ...
                tagged_long = [
                    dict(
                        r,
                        dir="LONG",
                        status="PASS" if r["score"] >= eff_min_score else "low score",
                    )
                ...
                _bot_state.last_scanner_results = sorted(
                    tagged_long + tagged_short, key=lambda x: x["score"], reverse=True
                )
            ...
            fresh_short = [
                res for res in short_results if is_result_fresh(res, now_utc)
            ]

            # ── Pick candidates ──────────────────────────────────────
            with _cache_lock:
                in_pos_updated = {p["symbol"] for p in _cached_positions}
                available_updated = get_dynamic_max_positions(_cached_balance) - len(
                    _cached_positions
                )
            ...
            if candidates:
                for result, direction in candidates:
                    with _cache_lock:
                        ...
                        if len(_cached_positions) >= get_dynamic_max_positions(
                            _cached_balance
                        ):
                            # REF: Tier 3: Temporal Inconsistency
                            make_entity_request(
                            ...
                            tui_log(
                                f"SKIP: {result['inst_id']} - Max positions reached while processing candidates.",
                                event_type="SKIP",
                            )
                    ...
                    verified_result = verify_candidate(
                        result["inst_id"], direction, result["score"]
                    )
...
def one_shot(args):
    ...
    print(
        f" Balance: {f'{balance:.2f}' if balance is not None else '?'} USDT | "
    ...
    if not args.no_dynamic:
        ...
        if eff_min_score > args.min_score:
            print(
                Fore.YELLOW + f" [ADAPTIVE FILTER] Dynamic Min Score: {eff_min_score}"
            )
    ...
    if candidates and not args.dry_run:
        ...
        if confirm == "y":
            ...
            # ── Wait & Verify ────────────────────────────────────
            verified_result = verify_candidate(
                top_result["inst_id"], top_dir, top_result["score"]
            )
            if verified_result:
...
def show_status():
    ...
    print(
        f" Margin    : ${MARGIN_USDT} @ {LEVERAGE}x = ${MARGIN_USDT*LEVERAGE:.0f} notional"
    )
    ...
    try:
        ...
        if trades:
            ...
            for t in trades[-5:][::-1]:
                ...
                print(
                    f"  {t['timestamp'][:19]} {t.get('direction','?'):5} "
...
def deploy_once(args):
    ...
    print(Fore.CYAN + Style.BRIGHT + f"{'='*70}")
    print(
        f" Balance: {balance:.2f} USDT | Available slots: {available_slots}/{MAX_POSITIONS}\n"
    )
    ...
    if deployed_summary:
        ...
        for s in deployed_summary:
            ...
            msg_lines.append(
                f"{emoji} *{s['symbol']}* @ {s['price']:.4g} (Stop: {s['stop']:.4g}) | Score: {s['score']}"
            )
        msg = msg_header + "\n".join(msg_lines)
        send_telegram_message(msg)
...
def verify_trailing_stops():
    ...
    resp = _get(
        "/g-orders/activeList", {"currency": "USDT", "ordStatus": "Untriggered"}
    )
    if resp and resp.get("code") == 0:
        ...
        trailing_stops = [
            order
            for order in resp.get("data", {}).get("rows", [])
            if order.get("ordType") == "Stop"
            and order.get("pegPriceType") == "TrailingStopPeg"
        ]
...
def main():
    if not API_KEY or not API_SECRET:
        print(
            Fore.RED
            + "[ERROR] PHEMEX_API_KEY and PHEMEX_API_SECRET must be set in .env"
        )
        print(" Example .env:")
    ...
    run_p.add_argument(
        "--min-score", type=int, default=MIN_SCORE, help="Min score to execute"
    )
    run_p.add_argument(
        "--min-score-gap",
        type=int,
        default=MIN_SCORE_GAP,
        help="Min score gap (Long - Short) for entry",
    )
    run_p.add_argument(
        "--min-signals",
        type=int,
        default=3,
        help="Minimum number of signals required to consider a candidate (default: 3)",
    )
    run_p.add_argument(
        "--direction",
        default=DIRECTION,
        choices=["LONG", "SHORT", "BOTH"],
        help="Direction to trade",
    )
    run_p.add_argument("--timeframe", default=TIMEFRAME, help="Scanner timeframe")
    run_p.add_argument(
        "--cooldown", type=int, default=4, help="Cooldown in candles after exit"
    )
    run_p.add_argument(
        "--min-vol", type=int, default=MIN_VOLUME, help="Min 24h USDT volume"
    )
    run_p.add_argument(
        "--workers", type=int, default=MAX_WORKERS, help="Scanner threads"
    )
    run_p.add_argument(
        "--rate", type=float, default=RATE_LIMIT_RPS, help="API requests/sec"
    )
    ...
    run_p.add_argument(
        "--no-dynamic", action="store_true", help="Disable adaptive score filtering"
    )
    run_p.add_argument(
        "--no-tui",
        action="store_true",
        help="Run without the full-screen TUI dashboard",
    )
    run_p.add_argument(
        "--web", action="store_true", help="Enable web dashboard backend"
    )
    run_p.add_argument(
        "--web-port", type=int, default=8081, help="Port for web dashboard backend"
    )

    # ── deploy: single scan + exit ───────────────────────────────────
    deploy_p = sub.add_parser(
        "deploy", help="Run one scan and optionally execute, then exit"
    )
    ...
    deploy_p.add_argument(
        "--direction", default=DIRECTION, choices=["LONG", "SHORT", "BOTH"]
    )
    ...
    deploy_p.add_argument(
        "--dry-run", action="store_true", help="Print orders but don't execute"
    )
    ...
    # ── status ──────────────────────────────────────────────────────
    sub.add_parser(
        "status", help="Show account balance, open positions, and recent trades"
    )
    ...
    if args.command in ["run", "deploy", "once"]:
        ...
        if not _SCANNERS_OK:
            print(
                Fore.RED + f"[ERROR] Could not import scanner modules: {_SCANNER_ERR}"
            )
            print(
                "Scanner modules (phemex_long.py/phemex_short.py) are required for this command."
            )
            sys.exit(1)
        ...
        logger.info(
            f"Cooldown set to {BLACKLIST_DURATION_SECONDS}s ({args.cooldown} candles)"
        )
    ...
    make_entity_request(
        ...
        data={
            ...
            "config": {
                ...
                "trail_pct": TRAIL_PCT,
                "min_score": (
                    args.min_score if hasattr(args, "min_score") else MIN_SCORE
                ),
                "min_score_gap": (
                    args.min_score_gap
                    if hasattr(args, "min_score_gap")
                    else MIN_SCORE_GAP
                ),
                "direction": (
                    args.direction if hasattr(args, "direction") else DIRECTION
                ),
                "timeframe": (
                    args.timeframe if hasattr(args, "timeframe") else TIMEFRAME
                ),
            },
            "status": "STARTED",
        },
    ...
    if args.command == "status":
        ...
        env_label = (
            Fore.YELLOW + "⚠ TESTNET"
            if testnet
            else Fore.RED + "🚨 MAINNET — REAL MONEY"
        )
        ...
        print(
            f" Min Score : {args.min_score} | Min Gap: {args.min_score_gap} | Direction: {args.direction}"
        )
        if args.dry_run:
