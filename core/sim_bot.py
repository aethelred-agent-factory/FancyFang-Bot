...
def pick_sim_leverage(
    atr_stop_pct: float | None, vol_spike: float = 1.0, is_low_liq: bool = False
) -> int:
    """
...
HAWKES_INTENSITY_CRITICAL = float(
    os.getenv("HAWKES_CRITICAL", "4.0")
)  # -> +0.50 penalty
...
class SimBotState:
    ...
    def load_account(self):
        ...
        with self.file_io_lock:
            if PAPER_ACCOUNT_FILE.exists():
                ...
                try:
                    db_check = self.storage.load_account(
    ...
    def save_account(self):
        ...
        with self.file_io_lock:
            ...
            try:
                ...
                PAPER_ACCOUNT_FILE.write_text(
                    json.dumps({"balance": balance, "positions": positions})
                )
            except Exception as error:
                logging.getLogger("sim_bot").error(
                    f"Failed to save paper account JSON: {error}"
                )
            ...
            try:
                ...
                logging.getLogger("sim_bot").error(
                    f"Failed to save paper account to DB: {error}"
                )
    ...
    def restore_from_snapshot(self, snapshot: Dict[str, Any]):
        ...
        with self.lock:
            ...
            self.fast_track_cooldowns = snapshot.get(
                "fast_track_cooldowns", self.fast_track_cooldowns
            )
...
def _get_session_chart() -> Optional[str]:
    ...
    try:
        ...
        plt.title(
            f"Sim Session Equity ({datetime.datetime.now().strftime('%Y-%m-%d %H:%M')})"
        )
        plt.xlabel("Points")
...
def save_sim_cooldowns() -> None:
    ...
    with state.lock:
        ...
        active_exit = {
            ...
            if time.time() - info[0] < MAX_COOLDOWN_S
        }
        active_ft = {
            s: ts
            for s, ts in state.fast_track_cooldowns.items()
            if time.time() - ts < FAST_TRACK_COOLDOWN_SECONDS
        }

    data = {"last_exit_info": active_exit, "fast_track": active_ft}
    try:
    ...
    try:
        ...
        with state.lock:
            state.last_exit_info = {
                s: (float(info[0]), float(info[1]))
                for s, info in exit_data.items()
                if time.time() - float(info[0]) < MAX_COOLDOWN_S
            }
            state.fast_track_cooldowns = {
                s: float(ts)
                for s, ts in ft_data.items()
                if time.time() - float(ts) < FAST_TRACK_COOLDOWN_SECONDS
            }
        logger.info(
            f"Loaded {len(state.last_exit_info)} exit and {len(state.fast_track_cooldowns)} fast-track cooldowns."
        )
...
def _ws_on_open(ws: websocket.WebSocketApp) -> None:
    ...
    if symbols:
        ws.send(
            json.dumps({"id": 1, "method": "market24h_p.subscribe", "params": symbols})
        )
...
def _ws_heartbeat(ws: websocket.WebSocketApp, stop_event: threading.Event) -> None:
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            logger.debug(
                "WebSocket closed during heartbeat — exiting heartbeat thread."
            )
            break
        except Exception as e:
...
def _ws_run_loop() -> None:
    ...
    ws_url = (
        ...
        if "testnet" in pc.BASE_URL
        else "wss://ws.phemex.com"
    )
    ...
    while True:
        ...
        state.ws_app = websocket.WebSocketApp(
            ws_url, on_message=_ws_on_message, on_open=_ws_on_open
        )
        threading.Thread(
            target=_ws_heartbeat, args=(state.ws_app, stop_event), daemon=True
        ).start()
        ...
        logger.info(
            f"WebSocket disconnected. Retrying in {delay}s (attempt {retries})..."
        )
        time.sleep(delay)
    ...
    def _do_sub() -> None:
        ...
        try:
            ...
            if state.ws_app and state.ws_app.sock and state.ws_app.sock.connected:
                ...
                state.ws_app.send(
                    json.dumps(
                        {"id": 1, "method": "market24h_p.subscribe", "params": symbols}
                    )
                )
        except Exception as e:
            logger.error(
...
def _check_stops_live(symbol: str) -> None:
    ...
    with state.lock:
        positions = state.positions
        pos_idx = next(
            (i for i, p in enumerate(positions) if p["symbol"] == symbol), None
        )
        if pos_idx is None:
            return
    ...
    if exit_to_process:
        ...
        tui_log(
            f"{exit_to_process['exit_reason'].upper()} HIT: {symbol} {exit_to_process['side']} closed at {exit_to_process['exit_price']}",
            event_type="EXIT",
        )
        ...
        if exit_to_process["entry_time"]:
            try:
                entry_time_dt = datetime.datetime.fromisoformat(
        ...
        dur_str = (
            f"{h_hour}h {h_min}m"
            if h_hour > 0
            else (f"{h_min}m {h_sec}s" if h_min > 0 else f"{h_sec}s")
        )
        ...
        _log_closed_trade(
            symbol,
            exit_to_process["side"],
            exit_to_process["entry"],
            exit_to_process["exit_price"],
            exit_to_process["size"],
            exit_to_process["entry_score"],
            exit_to_process["entry_time"],
    ...
    if entry_time:
        try:
            ...
            hold_time_seconds = int(
                (
                    datetime.datetime.now(datetime.timezone.utc) - entry_time_dt
                ).total_seconds()
            )
    ...
    with term.fullscreen(), term.cbreak(), term.hidden_cursor():
        while state.display_thread_running:
            ...
            print(
                ...
                + term.bold_white(
                    f" {Fore.MAGENTA}{pulse}{Style.RESET_ALL} SIMULATION DASHBOARD | {curr_time} UTC"
                )
            )
            ...
            if not positions:
                draw_panel(
                    ...
                    ui.modern_panel(
                        "ACTIVE POSITIONS",
                        [Fore.WHITE + " (Monitoring for signals...)"],
                        width=left_w,
                    ),
                )
                y += 4
                ...
                for pos in positions[:max_show]:
                    ...
                    if not now:
                        draw_panel(
                            ...
                            y,
                            ui.modern_panel(
                                sym, ["Waiting for price..."], width=left_w
                            ),
                        )
                        y += 4
                        continue
                    ...
                    draw_panel(
                        ...
                        y,
                        ui.glow_panel(
                            header, info, width=left_w, color_rgb=(255, 0, 255)
                        ),
                    )
                    y += len(info) + 2
            ...
            draw_panel(
                2,
                logs_y,
                ui.modern_panel("LOGS", log_lines, width=left_w, color=Fore.WHITE),
            )
            ...
            if history:
                ...
                hist_lines.append(
                    f"PnL: {ui.pnl_color(tot)}{tot:+.2f}{Style.RESET_ALL}"
                )
                ...
                for t in reversed(history[-avail_h:]):
                    ...
                    hist_lines.append(
                        f"{ts} {s:<6} {ui.pnl_color(p)}{p:+.2f}{Style.RESET_ALL}"
                    )

            draw_panel(
                left_w + 4,
                start_y,
                ui.modern_panel("HISTORY", hist_lines, width=right_w),
            )
...
def check_opposite_signal(
    symbol: str, side: str, ticker: Optional[dict] = None
) -> Tuple[bool, int]:
    """
...
def update_pnl_and_stops(args) -> None:
    ...
    with state.lock:
        ...
        missing = [
            p["symbol"] for p in positions if p["symbol"] not in state.live_prices
        ]
        if missing:
    ...
    with state.lock:
        ...
        for pos in state.positions:
            ...
            pos["pnl"] = (
                (current_price - pos["entry"]) * pos["size"]
                if side == "Buy"
                else (pos["entry"] - current_price) * pos["size"]
            )
            new_positions.append(pos)
    ...
    for ex in exits_to_process:
        ...
        tui_log(
            f"{ex['exit_reason'].upper()} HIT: {symbol} closed at {ex['exit_price']}"
        )
...
def verify_sim_candidate(
    symbol: str, direction: str, original_score: int, wait_seconds: int = 20
) -> Optional[dict]:
    """
    ...
    for i in range(steps):
        ...
        if current_rsi:
            if direction == "LONG" and current_rsi > 70:
                tui_log(
                    f"FAIL: {symbol} RSI {current_rsi:.1f} — overbought after wait."
                )
                return None
...
def execute_sim_setup(result: dict, direction: str, args: Any) -> bool:
    ...
    if _DD_OK:
        ...
        if state.balance <= 0:
            tui_log(
                f"FATAL: {symbol} blocked — account balance ${state.balance:.2f} is invalid",
                event_type="SKIP",
            )
            return False
        allowed, dd_reason = drawdown_guard.can_open_trade(
    ...
    if _TG_OK and telegram.is_halted():
        tui_log(
            f"TG HALT: {symbol} blocked — /stop was issued via Telegram",
            event_type="SKIP",
        )
        return False
    ...
    with state.lock:
        ...
        if existing_pos:
            ...
            if existing_pos.get("margin", 0.0) >= MAX_MARGIN_PER_SYMBOL:
                tui_log(
                    f"SKIP: {symbol} at MAX MARGIN (${existing_pos['margin']:.2f})",
                    event_type="SKIP",
                )
                return False
            ...
            if last_exit_info and len(last_exit_info) == 2:
                ...
                dynamic_cooldown = _calculate_dynamic_cooldown(
                    last_pnl, state.entropy_penalty
                )

                if time.time() - last_exit_timestamp < dynamic_cooldown:
                    remaining_s = dynamic_cooldown - (time.time() - last_exit_timestamp)
                    tui_log(
                        f"COOLDOWN: {symbol} — {remaining_s/60:.1f}m remaining (PnL: {last_pnl:.2f}, Penalty: {state.entropy_penalty})",
                        event_type="SKIP",
                    )
                    return False
        ...
        if _CM_OK:
            blocked, reason = corr_mgr.correlation_mgr.should_block_entry(
                symbol, direction, state.positions
            )
            if blocked:
        ...
        # Tiered score gate
        effective_min = (
            pc.SCORE_MIN_LOW_LIQ
            if is_low_liq
            else pc.SCORE_MIN_HTF_BYPASS if is_htf else pc.SCORE_MIN_DEFAULT
        )
    ...
    if _RISK_MGR_OK:
        ...
        tui_log(
            f"{symbol}: LOW-LIQ MODE — {active_leverage}x lev, "
        ...
        if getattr(args, "margin", None):
            ...
            avg_win = (
                current_stats["win_pnl"] / current_stats["wins"]
                if current_stats["wins"] > 0
                else 1.0
            )
            avg_loss = (
                abs(current_stats["loss_pnl"] / current_stats["losses"])
                if current_stats["losses"] > 0
                else 1.0
            )
    ...
    active_leverage = getattr(
        args, "leverage", pick_sim_leverage(atr_stop_pct, vol_spike, is_low_liq)
    )
    active_trail_pct = p_bot.LOW_LIQ_TRAIL_PCT if is_low_liq else p_bot.TRAIL_PCT
    tui_log(
        f"LEV SELECT: {symbol} → {active_leverage}x (ATR%={atr_stop_pct}, spike={vol_spike:.1f}x, low_liq={is_low_liq})",
        event_type="INFO",
    )
    ...
    tui_log(
        f"SLIPPAGE: {symbol} mid={price:.6g} fill={fill_price:.6g} ", event_type="FILL"
    )
    ...
    if getattr(args, "stop_loss_pct", None):
        ...
        tui_log(
            f"FIXED STOP: {symbol} stop={stop_px:.6g} ({args.stop_loss_pct * 100:.1f}%)",
            event_type="STOP",
        )
    ...
    tui_log(
        f"{action_label} {arrow} {symbol} @ {entry_price:.6g} (Score: {score}) slippage={slippage_amt:.6g}"
    )
...
def on_scan_result(scan_res: dict, direction: str, args: Any) -> None:
    ...
    effective_fast_track = min(
        p_bot.FAST_TRACK_PREDICTIVE_SCORE + hawkes_penalty + current_penalty, 200
    )
    if predictive_score < effective_fast_track:
        if hawkes_penalty > 0 or current_penalty > 0:
            tui_log(
                f"FT THROTTLE: {scan_res['inst_id']} pred_score {predictive_score:.2f} < dynamic FT threshold {effective_fast_track:.2f} (λ={intensity:.2f}, H_pen={current_penalty:.2f})"
            )
        return
    ...
    with state.lock:
        ...
        pending_margin = len(state.fast_track_opened) * (
            p_bot.MARGIN_USDT * 1.05
        )  # buffer for in-flight verifications
        ...
        if (
            scan_res["inst_id"] in current_syms
            or scan_res["inst_id"] in state.fast_track_opened
        ):
            return
    ...
    tui_log(
        f"⚡ FAST-TRACK: {scan_res['inst_id']} pred_score {predictive_score:.2f}! (λ={intensity:.2f})"
    )

    # ── Wait & Verify ────────────────────────────────────
    try:
        verified_result = verify_sim_candidate(
            scan_res["inst_id"], direction, predictive_score
        )
        if verified_result:
        ...
        tui_log(
            f"CRITICAL ERROR in on_scan_result for {scan_res['inst_id']}: {e}",
            event_type="ERROR",
        )
...
def sim_bot_loop(args) -> None:
    ...
    logger.info(
        f"Simulation cooldown set to {COOLDOWN_SECONDS}s ({args.cooldown} candles)"
    )
    ...
    cfg = {
        ...
        "no_htf": getattr(args, "no_htf", False),
    ...
    try:
        ...
        logger.error(
            f"Failed to load sim_state snapshot, falling back to account-only load: {e}"
        )
        snapshot = None
    ...
    while _running:
        try:
            ...
            state.display_paused_event.set()
            t0 = time.time()
            long_r, short_r = p_bot.run_scanner_both(
                cfg, args, on_result=lambda r, d: on_scan_result(r, d, args)
            )
            elapsed = time.time() - t0
            state.display_paused_event.clear()
            tui_log(
                f"Scan complete in {elapsed:.1f}s — L: {len(long_r)}  S: {len(short_r)}"
            )
            ...
            if total_universe > 0 and n_hits > 0:
                ...
                new_entropy_penalty = min(
                    ENTROPY_MAX_PENALTY, sat_penalty + side_penalty
                )
            ...
            if new_entropy_penalty > ENTROPY_ALERT_LEVEL:
                tui_log(
                    f"ENTROPY DEFLATOR: Raising min_score by +{new_entropy_penalty} (Saturation: {n_hits}/{total_universe}, Imbalance: {imbalance:.2f})"
                )
            ...
            if eff_min_score > args.min_score:
                tui_log(
                    f"ADAPTIVE FILTER: Effective min_score = {eff_min_score} (Penalty: +{new_entropy_penalty})"
                )
            ...
            if getattr(args, "symbols", None):
                ...
                candidates = [
                    c for c in candidates if c[0].get("inst_id", "").upper() in wanted
                ]
            ...
            if candidates:
                ...
                for res, direction in candidates:
                    ...
                    with state.lock:
                        ...
                        eff_free -= len(state.fast_track_opened) * (
                            p_bot.MARGIN_USDT * 1.05
                        )
                    ...
                    try:
                        verified_result = verify_sim_candidate(
                            res["inst_id"], direction, res["score"]
                        )
                        if verified_result:
                            execute_sim_setup(verified_result, direction, args)
                    except Exception as e:
                        ...
                        tui_log(
                            f"CRITICAL ERROR in candidate loop for {res['inst_id']}: {e}",
                            event_type="ERROR",
                        )
                        logger.error(traceback.format_exc())
...
def main() -> None:
    ...
    parser.add_argument(
        "--cooldown", type=int, default=5, help="Cooldown in candles after exit"
    )
    ...
    parser.add_argument(
        "--csv", action="store_true", help="Log all trades to a CSV file"
    )
    ...
    parser.add_argument(
        "--textual", action="store_true", help="Use modern Textual TUI dashboard"
    )
    parser.add_argument(
        "--web", action="store_true", help="Enable web dashboard backend"
    )
    parser.add_argument(
        "--web-port", type=int, default=8080, help="Port for web dashboard backend"
    )
    parser.add_argument(
        "--no-tui",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Number of historical candles to fetch per scan",
    )
    parser.add_argument(
        "--symbols",
        type=str,
        default=None,
        help="Comma-separated list of symbols to restrict to (e.g. BTCUSDT,ETHUSDT)",
    )
    parser.add_argument(
        "--min-signals",
        type=int,
        default=3,
        help="Minimum number of signals required for a candidate",
    )
    parser.add_argument(
        "--margin",
        type=float,
        default=p_bot.MARGIN_USDT,
        help="Margin (USDT) to use per trade",
    )
    parser.add_argument(
        "--max-margin",
        type=float,
        default=MAX_MARGIN_PER_SYMBOL,
        help="Max margin per symbol allowed in sim",
    )
    parser.add_argument(
        "--stop-loss-pct",
        type=float,
        default=p_bot.TRAIL_PCT,
        help="Trailing stop percent (e.g. 0.03 for 3%%)",
    )
    parser.add_argument(
        "--take-profit-pct",
        type=float,
    ...
    print(
        Fore.GREEN
        + Style.BRIGHT
        + "  🚀 Phemex SIMULATION Bot Starting (Paper Trading)"
    )
    ...
    print(
        f"  Score    : {args.min_score} (gap: {args.min_score_gap})  Direction: {args.direction}\n"
    )
    ...
    if _DD_OK:
        ...
        logger.info(
            f"drawdown_guard: active — max daily drawdown {drawdown_guard.MAX_DAILY_DRAWDOWN:.1%}"
        )
        ...
        def _session_pnl_fn():
            with state.lock:
                return {
                    ...
                    "total_pnl": state.rolling_stats["win_pnl"]
                    + state.rolling_stats["loss_pnl"],
                }
    ...
    try:
        if args.textual and not args.no_tui:
            ...
            bot_thread = threading.Thread(
                target=sim_bot_loop, args=(args,), daemon=True
            )
            bot_thread.start()

            # Launch Textual App (blocking)
            app = FancyBotApp(
                bot_state=state, bot_logs=_bot_logs, initial_balance=INITIAL_BALANCE
            )
            app.run()
