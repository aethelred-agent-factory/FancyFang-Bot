...
class TestMemeReaperV21(unittest.TestCase):
    def test_ema200_calculation_in_unified_analyse(self):
        ...
        candles = [[i, 14400, 50000, 50000, 51000, 49000, 50000, 100] for i in range(500)]

        with patch("core.phemex_common.get_candles", return_value=candles), \
             patch("core.phemex_common.get_funding_rate_info", return_value=(0.0001, 0.0001, 0.0)), \
             patch("core.phemex_common.get_order_book_with_volumes", return_value=(49999, 50001, 0.01, 1000, 1.0)):

            # Use a dummy score function
            def dummy_score(data): return 100, ["Signal"]
            def dummy_patterns(ohlc): return []
            def dummy_div(closes, rsi): return False
            def dummy_conf(data, score, bb): return "HIGH", "green", []

            result = pc.unified_analyse(
                ticker, cfg, "SHORT", dummy_score, dummy_patterns, dummy_div, dummy_conf,
                enable_ai=False, enable_entity=False
            )
    ...
    def test_score_short_ema200_divergence(self):
        ...
        data = pc.TickerData(
            inst_id="BTCUSDT", price=55000.0, rsi=70.0, prev_rsi=71.0,
            bb={"upper": 54000.0, "lower": 50000.0, "mid": 52000.0, "width_pct": 5.0},
            ema21=52000.0, change_24h=5.0, funding_rate=0.0002, patterns=[],
    ...
    def test_score_long_ema200_divergence(self):
        ...
        data = pc.TickerData(
            inst_id="BTCUSDT", price=45000.0, rsi=30.0, prev_rsi=29.0,
            bb={"upper": 50000.0, "lower": 46000.0, "mid": 48000.0, "width_pct": 5.0},
            ema21=48000.0, change_24h=-5.0, funding_rate=-0.0002, patterns=[],
    ...
    @patch("core.phemex_common.get_tickers")
    @patch("core.phemex_short.analyse")
    def test_verify_candidate_fails_on_score_drop(self, mock_analyse, mock_get_tickers, mock_sleep):
        mock_get_tickers.return_value = [{"symbol": "BTCUSDT", "lastRp": "50000"}]
