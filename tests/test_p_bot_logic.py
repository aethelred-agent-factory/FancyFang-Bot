...
def test_pick_candidates():
    long_res = [
        {"inst_id": "BTC", "score": 150, "signals": ["A", "B", "C"]},
        {"inst_id": "ETH", "score": 120, "signals": ["A", "B", "C"]},
    ]
    short_res = [
        {"inst_id": "BTC", "score": 100, "signals": ["A", "B", "C"]},
        {"inst_id": "SOL", "score": 140, "signals": ["A", "B", "C"]},
    ]
...
def test_round_qty():
    ...
    with (patch("core.p_bot._instrument_cache", {"BTCUSDT": {"step": 0.001}}), 
          patch("core.p_bot._instrument_loaded", True)):
        assert p_bot._round_qty("BTCUSDT", 0.00567) == "0.005"
        
    with (patch("core.p_bot._instrument_cache", {"ETHUSDT": {"step": 0.01}}), 
          patch("core.p_bot._instrument_loaded", True)):
