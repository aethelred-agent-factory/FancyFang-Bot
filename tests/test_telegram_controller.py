...
def mock_requests():
    with patch("requests.get") as mock_get, patch("requests.post") as mock_post, patch(
        "modules.telegram_controller.TG_BOT_TOKEN", "token"
...
def test_telegram_controller_get_updates(mock_requests):
    ...
    mock_get.return_value.json.return_value = {
        "result": [
            {
                "update_id": 100,
                "message": {"text": "/status", "chat": {"id": "chat_id"}},
            }
        ]
    }
...
def test_handle_status(mock_requests):
    ...
    with (
        patch(
            "modules.drawdown_guard.get_status",
            return_value={
                "daily_pnl": 10.0,
                "loss_pct": 0.01,
                "killed": False,
...
def test_handle_profit(mock_requests):
    mock_get, mock_post = mock_requests
    telegram_controller._get_session_pnl = lambda: {
        "wins": 2,
        "losses": 1,
        "total_pnl": 15.0,
    }
...
def test_poll_loop_iteration(mock_requests):
    ...
    mock_get.return_value.json.return_value = {
        "result": [
            {"update_id": 100, "message": {"text": "/start", "chat": {"id": "chat_id"}}}
        ]
    }
