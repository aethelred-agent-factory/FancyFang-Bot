...
def test_log_closed_trade_warning(tmp_path, caplog, monkeypatch):
    ...
    fake_storage.load_account.return_value = {"balance": 100.0, "positions": [{"symbol": "FOO"}]}
...
def test_sim_account_isolation(tmp_path, monkeypatch, caplog):
    ...
    p_storage.save_account_state(1000.0, [{"symbol": "LIVE", "side": "Buy", "size": 1.0, "margin": 10.0, "entry": 100.0}])
    ...
    sim_state.positions = [{"symbol": "SIM", "side": "Sell", "size": 2.0, "margin": 20.0, "entry": 200.0}]
