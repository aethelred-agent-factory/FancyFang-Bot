...
from core.phemex_long import detect_bullish_divergence
from core.phemex_long import detect_patterns as detect_patterns_long
from core.phemex_long import feature_builder_long, score_long
from core.phemex_short import detect_bearish_divergence
from core.phemex_short import detect_patterns as detect_patterns_short
...
def score_window_unified(
    ...
    kalman_slope = (
        kalman_series[-1] - kalman_series[-2] if len(kalman_series) >= 2 else 0.0
    )
    ...
    data = pc.TickerData(
        ...
        bb=bb,
        ema21=ema21,
        change_24h=pc.pct_change(last, open_24h),
        funding_rate=funding,
        patterns=patterns,
        dist_low_pct=pc.pct_change(last, low_24h),
        dist_high_pct=pc.pct_change(last, high_24h),
        vol_spike=vol_spike,
        has_div=has_div,
        rsi_1h=rsi_1h,
        fr_change=0.0,
...
def print_stats(trades: List[Trade], label: str = "", timeframe: str = "15m"):
    ...
    lines = [
        ...
        ui.cyber_telemetry(
            "Sharpe", sharpe_ratio if not np.isnan(sharpe_ratio) else 0.0, 3.0, ""
        ),
        ui.cyber_telemetry(
            "Sortino", sortino_ratio if not np.isnan(sortino_ratio) else 0.0, 5.0, ""
        ),
        "",
    ...
    print(
        "\n"
        + ui.glow_panel(
            f"MISSION REPORT: {label or 'BACKTEST RESULTS'}",
            lines,
            color_rgb=(255, 128, 0),
            width=80,
        )
...
def main():
    ...
    parser.add_argument(
        "--margin", type=float, default=10.0, help="USDT margin per trade"
    )
    parser.add_argument(
        "--max-margin",
        type=float,
