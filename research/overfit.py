...
def _run(sym_data, kwargs) -> List[bt.Trade]:
    ...
    for sym, candles, spread, funding, rsi_1h in sym_data:
        ...
        kw.pop("debug", None)
        all_trades.extend(
            bt.backtest_symbol(sym, candles, spread, funding, rsi_1h, **kw)
        )
    return all_trades
...
def run_final_verdict(regime, perm, rand_ent, sens) -> dict:
    ...
    p = regime.get(
        "profitable", sum(1 for s in regime.get("slices", []) if s.get("pnl", 0) > 0)
    )
    cv = regime.get("cv", 0)
    if p == 4 and cv < 0.8:
        checks.append(
...
def main():
    ...
    def fetch(sym):
        ...
        with lock:
            ...
            if args.debug:
                print(
                    f"[debug] Fetched {done[0]}/{len(symbols)}: {sym} (candles={len(candles)})"
                )
...
def print_pretty(results: dict) -> None:
    ...
    print(
        f"{CYAN}{BOLD}  ║  FancyFangBot OVERFIT DIAGNOSTIC                   ║{RESET}"
    )
    print(
        f"{CYAN}{BOLD}  ║  Regime · Permutation · Random Entry · Sensitivity ║{RESET}"
    )
    print(
        f"{CYAN}{BOLD}  ╚═══════════════════════════════════════════════════╝{RESET}\n"
    )
    ...
    for m, label in zip(slices, labels):
        ...
        print(
            f"  {label:<14} {m.get('trades',0):>7} {m.get('win_rate',0.0):>6.1f}% {pnl_col}{pnl:>+12.4f}{RESET} {m.get('expectancy',0.0):>+9.4f}   {pnl_col}{bar}{RESET}"
        )
    print()
    ...
    print(
        f"  Real PnL : {GREEN if perm.get('real_pnl',0)>0 else RED}{perm.get('real_pnl',0):+10.4f}{RESET}"
    )
    print(
        f"  Null mean: {perm.get('null_mean',0):+10.4f}  std={perm.get('null_std',0):.4f}  p={perm.get('p_value',0):.3f}  z={perm.get('z_score','nan')}"
    )
    ...
    print(
        f"  Random mean PnL: {rand.get('mean_random_pnl',0):+10.4f}  std={rand.get('std_random_pnl',0):.4f}"
    )
    print(
        f"  Profitable runs: {rand.get('profitable_random_runs',0)}/{rand.get('total_random_runs',0)}"
    )
    print(
        f"  Real PnL: {GREEN if rand.get('real_pnl',0)>0 else RED}{rand.get('real_pnl',0):+10.4f}{RESET}"
    )
    print(
        f"  Signal-only edge: {rand.get('signal_contribution',0):+10.4f}  ({rand.get('random_pct_of_real',0):.0f}% random) "
    )
    ...
    print(
        f"  Base PnL: {sens.get('base_pnl',0):+10.4f}  Verdict: {sens.get('verdict')}"
    )
    print(
        f"  {'Perturbation':<34} {'PnL':>12}   {'WR%':>7}   {'Δ PnL':>11}   Stability"
    )
    print(f"  {'─'*78}")
    for p in sens.get("perturbations", []):
        ...
        pnl_col = GREEN if pnl > 0 else RED
        print(
            f"  {p.get('label'):<34} {pnl_col}{pnl:>+12.4f}{RESET}   {stab:>6}   {delta:>+11.4f}   {stab:>6}{id_note}"
        )
    print()
