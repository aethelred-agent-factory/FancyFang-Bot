...
def _get_session() -> requests.Session:
    ...
    if not hasattr(_thread_local, "session"):
        ...
        session.mount(
            "https://",
            HTTPAdapter(max_retries=retry, pool_connections=50, pool_maxsize=50),
        )
...
def score_long_window(
    ...
    dist_from_low = (
        calculate_percentage_change(current_price, low_24h) if low_24h > 0 else None
    )
    ...
    if ema200 is not None:
        if current_price < ema200:
            score -= 15
            signals.append(
                f"Price below EMA200 ({current_price:.2f} < {ema200:.2f}) — Downtrend"
            )
        else:
            score += 10
            signals.append(
                f"Price above EMA200 ({current_price:.2f} > {ema200:.2f}) — Uptrend"
            )
    ...
    if funding is not None and funding_prev is not None:
        ...
        if funding_change < 0:
            ...
            signals.append(
                f"Funding Momentum (becoming more negative {funding_change*100:.4f}% — squeeze building)"
            )
    ...
    if rsi is not None:
        ...
        if rsi < 25:
            ...
            signals.append(
                f"RSI {rsi:.1f} (oversold{' ✓ recovering' if rolling_up else ''})"
            )
    ...
    if bollinger is not None:
        ...
        if bb_range > 0:
            ...
            if bb_percentage <= 0.10:
                ...
                signals.append(
                    f"Price at/below BB lower ({bb_percentage*100:.0f}%) — extreme oversold"
                )
    ...
    if ema21 is not None:
        ...
        if pct_from_ema < -3.0:
            ...
            signals.append(
                f"Price {abs(pct_from_ema):.1f}% below EMA21 (mean-reversion opportunity)"
            )
    ...
    if funding is not None:
        ...
        if funding_rate_pct < -0.01:
            ...
            signals.append(
                f"Negative Funding ({funding_rate_pct:.4f}%) — crowded shorts, squeeze fuel"
            )
        elif funding_rate_pct > 0.10:
            score -= 10
            signals.append(
                f"Positive Funding ({funding_rate_pct:.4f}%) — crowded longs, caution"
            )
...
def score_short_window(
    ...
    change_24h = calculate_percentage_change(current_price, open_24h)
    dist_from_high = (
        calculate_percentage_change(high_24h, current_price)
        if current_price > 0
        else None
    )
    ...
    if len(ema_series) >= 4:
        ...
        if len(valid_emas) >= 2:
            ...
            if slope < 0:
                score += 12
                signals.append(
                    f"Negative EMA Slope ({slope:.4f}) — Downtrend confirmed"
                )
            elif slope > 0.01:
                score += 5
    ...
    if ema200 is not None:
        if current_price > ema200:
            score -= 15
            signals.append(
                f"Price above EMA200 ({current_price:.2f} > {ema200:.2f}) — Uptrend (risky short)"
            )
        else:
            score += 10
            signals.append(
                f"Price below EMA200 ({current_price:.2f} < {ema200:.2f}) — Downtrend (trend-aligned short)"
            )
    ...
    if funding is not None and funding_prev is not None:
        ...
        if funding_change > 0:
            score += weights["funding_momentum"]
            signals.append(
                f"Funding Momentum (becoming more positive +{funding_change*100:.4f}% — fade building)"
            )
    ...
    if rsi_1h is not None:
        if rsi_1h > 65:
            ...
            score += 8
            signals.append(
                f"HTF Alignment (1H RSI {rsi_1h:.1f}) — overbought territory"
            )
    ...
    if rsi is not None:
        ...
        if rsi > 75:
            ...
            signals.append(
                f"RSI {rsi:.1f} (rollover zone{' ✓ rolling over' if rolling_over else ''})"
            )
            score += pts
        elif rsi < 35:
            score -= 5
    ...
    if bollinger is not None:
        ...
        if bb_range > 0:
            ...
            if bb_percentage >= 0.90:
                ...
                signals.append(
                    f"Price at/above BB upper ({bb_percentage*100:.0f}%) — extreme overbought"
                )
                ...
                score -= 5
                signals.append(
                    f"Below BB mid — fading short ({bb_percentage*100:.0f}%)"
                )
    ...
    if ema21 is not None:
        ...
        if pct_from_ema > 3.0:
            score += weights["ema_stretch_3"]
            signals.append(
                f"Price {pct_from_ema:.1f}% above EMA21 (mean-reversion opportunity)"
            )
    ...
    if funding is not None:
        ...
        if funding_rate_pct > 0.10:
            ...
            signals.append(
                f"Funding +{funding_rate_pct:.4f}% (heavily crowded longs — fade primed)"
            )
            ...
            score -= 12
            signals.append(
                f"Funding {funding_rate_pct:.4f}% (crowded shorts — risky short)"
            )
...
# ─────────────────────────────────────────────────────────────────────
def _calculate_dynamic_cooldown_candles(
    pnl_usdt: float, base_win: int = 1, base_loss: int = 2, max_c: int = 8
) -> int:
...
def backtest_symbol(
    ...
    for candle_index in range(window, len(ohlcv_data) - 1):
        ...
        if is_in_position and active_position:
            ...
            if active_position.direction == "LONG":
                ...
                if take_profit_pct > 0:
                    ...
                    if current_high >= tp_level:
                        ...
                        active_position.pnl_usdt = (
                            raw_return
                            * active_position.leverage
                            * active_position.margin
                        )
                        active_position.exit_reason = "take_profit"
                        active_position.hold_candles = (
                            candle_index - active_position.entry_idx
                        )
                        ...
                        current_cooldown = _calculate_dynamic_cooldown_candles(
                            pnl_for_cooldown, base_win=1, base_loss=max(2, cooldown)
                        )
                        continue

                # Hard stop check (using candle low)
                if hard_stop_pct > 0:
                    hard_stop_level = active_position.entry_price * (
                        1.0 - hard_stop_pct
                    )
                    if current_low <= hard_stop_level:
                        exit_price = hard_stop_level * (
                            1.0 - slip_one_side - fee_one_side
                        )
                        raw_return = (
                            exit_price - active_position.entry_price
                        ) / active_position.entry_price
                ...
                if current_low <= stop_price:
                    ...
                    active_position.pnl_pct = raw_return * 100
                    active_position.pnl_usdt = (
                        raw_return * active_position.leverage * active_position.margin
                    )
                ...
                if take_profit_pct > 0:
                    ...
                    if current_low <= tp_level:
                        exit_price = tp_level * (1.0 + slip_one_side + fee_one_side)
                        raw_return = (
                            active_position.entry_price - exit_price
                        ) / active_position.entry_price
                        active_position.exit_idx = candle_index
                        active_position.exit_price = exit_price
                        active_position.pnl_pct = raw_return * 100
            ...
            if (
                is_in_position
                and active_position
                and (candle_index - active_position.entry_idx >= max_hold)
            ):
                ...
                if new_direction == active_position.direction:
                    if active_position.margin < max_margin:
                        ...
                        if next_open > 0:
                            ...
                            atr_w = _calc_atr_simple(
                                highs_window, lows_window, closes_window, 14
                            )
                            vol_s = vol_spike_ratio(vols_window)
                            unit_lev = pick_sim_leverage(
                                atr_w / next_open * 100.0 if atr_w else None,
                                vol_s,
                                is_low_liq,
                            )
                            ...
                            old_notional = (
                                active_position.size * active_position.entry_price
                            )
                            ...
                            atr_stop_dist = (
                                (atr_w * 1.5)
                                if atr_w
                                else (next_open * active_position.trail_pct)
                            )
                            if new_direction == "LONG":
        ...
        if (
            spread is not None and spread > 0.40
        ):  # Synchronized with pc.SPREAD_FILTER_MAX_PCT
        ...
        vol_s = vol_spike_ratio(vols_window)
        active_leverage = pick_sim_leverage(
            atr_window / next_open * 100.0 if atr_window else None, vol_s, is_low_liq
        )
    ...
    if (
        ...
        and candle_index > active_position.entry_idx
    ):
...
def compute_drawdown(trades: List[Trade]) -> Tuple[float, float]:
    ...
    max_drawdown_pct = (
        (maximum_drawdown / peak_equity * 100.0) if peak_equity > 0 else 0.0
    )
...
def sweep(
    ...
    for index, (
        ...
        min_score,
        leverage,
    ) in enumerate(grid_combinations):
        all_trades = []
        ...
        closed_trades = [
            ...
            if trade.pnl_usdt is not None and trade.exit_reason != "open"
        ]
        print(
            f"\r  Sweeping {index+1}/{len(grid_combinations)} — trail={tp_trail*100:.1f}% sl={stop_loss_pct*100:.1f}% tp={take_profit_pct*100:.1f}% score={min_score} lev={leverage}x"
        ...
        results.append(
            SweepResult(
                ...
                total_pnl=total_pnl,
                avg_win=(
                    float(np.mean([trade.pnl_usdt for trade in winning_trades]))
                    if winning_trades
                    else 0
                ),
                avg_loss=(
                    float(np.mean([trade.pnl_usdt for trade in losing_trades]))
                    if losing_trades
                    else 0
                ),
                profit_factor=(
                ...
                avg_hold=float(
                    np.mean([trade.hold_candles for trade in closed_trades])
                ),
...
def print_stats(trades: List[Trade], label: str = "", timeframe: str = "15m"):
    ...
    print(
        f"  Avg Win     : {Fore.GREEN}{avg_win:+.4f}{Style.RESET_ALL}  "
        f"Avg Loss: {Fore.RED}{avg_loss:+.4f}{Style.RESET_ALL}"
    )
    pf_color = (
        Fore.GREEN
        if profit_factor >= 1.5
        else (Fore.YELLOW if profit_factor >= 1.0 else Fore.RED)
    )
    ...
    sh_color = (
        Fore.GREEN
        if (not np.isnan(sharpe_ratio) and sharpe_ratio >= 1.0)
        else (
            Fore.YELLOW
    ...
    so_color = (
        Fore.GREEN
        if (not np.isnan(sortino_ratio) and sortino_ratio >= 1.5)
        else (
            Fore.YELLOW
            if (not np.isnan(sortino_ratio) and sortino_ratio >= 0)
            else Fore.RED
        )
    )
    print(
        f"  Max Drawdown: {dd_color}{max_drawdown:+.4f} USDT  ({max_drawdown_pct:.1f}%){Style.RESET_ALL}"
    )
    print(
        f"  Sharpe (ann): {sh_color}{fmt_stat(sharpe_ratio)}{Style.RESET_ALL}   "
    ...
    print(
        f"  Best Trade  : {Fore.GREEN}{best_trade.pnl_usdt:+.4f}{Style.RESET_ALL} "
        f"({best_trade.symbol} {best_trade.direction})"
    )
    print(
        f"  Worst Trade : {Fore.RED}{worst_trade.pnl_usdt:+.4f}{Style.RESET_ALL} "
        f"({worst_trade.symbol} {worst_trade.direction})"
    )
    print()

    # Direction breakdown
    for dir_label, group in [
        ("LONG", [t for t in closed_trades if t.direction == "LONG"]),
        ...
        print(
            f"  {dir_label:<6}: {len(group):3} trades | WR {g_wr:.0f}% "
    ...
    for reason in ["trail_stop", "hard_stop", "take_profit", "max_hold", "end_of_data"]:
        ...
        print(
            f"  {reason:<14}: {len(group):3} trades | WR {g_wr:.0f}% | PnL {rc}{g_pnl:+.4f}{Style.RESET_ALL}"
        )
    ...
    for lo, hi, tlabel in tiers:
        ...
        print(
            f"  {tlabel:>8}: [{wc}{draw_bar(g_wr)}{Style.RESET_ALL}] "
    ...
    for slabel, patterns in signal_groups:
        ...
        def has_signal(t, pats=patterns):
            return any(
                any(re.search(p, s, re.IGNORECASE) for p in pats) for s in t.signals
            )
        ...
        ec = Fore.GREEN if g_exp >= 0 else Fore.RED
        print(
            f"  {slabel:<22}: {wc}{g_wr:4.0f}% WR{Style.RESET_ALL} "
    ...
    for ll_label, ll_val in [("Normal liquidity", False), ("Low liquidity", True)]:
        ...
        print(
            f"  {ll_label:<22}: WR {g_wr:.0f}% | PnL {lc}{g_pnl:+.4f}{Style.RESET_ALL} | n={len(group)}"
        )
...
def print_per_symbol_stats(trades: List[Trade], top_n: int = 20):
    ...
    print(
        Fore.CYAN
        + Style.BRIGHT
        + f"  PER-SYMBOL BREAKDOWN  (top/bottom {top_n}, sorted by PnL)"
    )
    print(Fore.CYAN + f"{'═'*70}")
...
def print_sweep_results(results: List[SweepResult], top_n: int = 15):
    ...
    print(
        f"  {'Trail%':>6} {'SL%':>6} {'TP%':>6} {'MinScore':>9} {'Lev':>4} {'Trades':>7} "
    ...
    for r in results[:top_n]:
        ...
        pf_c = (
            ...
            else (Fore.YELLOW if r.profit_factor >= 1.0 else Fore.RED)
        )
...
def main():
    ...
    # ── Core settings ─────────────────────────────────────────────────
    parser.add_argument(
        "--symbols", nargs="+", default=[], help="Specific symbols to test"
    )
    parser.add_argument("--timeframe", default="4H")
    parser.add_argument(
        "--candles", type=int, default=500, help="Historical candles per symbol"
    )
    ...
    parser.add_argument(
        "--margin", type=float, default=5.0, help="USDT margin per trade"
    )
    parser.add_argument(
        "--max-margin",
        type=float,
    ...
    parser.add_argument(
        "--max-hold", type=int, default=96, help="Max candles to hold a trade"
    )
    parser.add_argument("--min-vol", type=int, default=5_000_000)
    parser.add_argument(
        "--workers", type=int, default=30, help="Parallel fetch workers"
    )
    ...
    # ── Sweep ─────────────────────────────────────────────────────────
    parser.add_argument("--sweep", action="store_true", help="Run parameter grid sweep")
    parser.add_argument(
        "--sweep-n", type=int, default=25, help="Symbol count for sweep"
    )
    ...
    print(
        Fore.WHITE + f"  Fetching {args.candles}x {args.timeframe} candles"
        f"{' + 1H RSI' if not args.no_htf else ''}..."
    )
    print(
        Fore.WHITE
        + f"  (This takes ~{max(5, len(symbols)//3)}s with {args.workers} workers)\n"
    )
    ...
    def fetch(sym):
        ...
        with lock:
            ...
            done_count[0] += 1
            print(
                f"\r  Fetching data: {done_count[0]}/{len(symbols)} symbols",
                end="",
                flush=True,
            )
    ...
    if not valid:
        ...
        print(
            Fore.YELLOW
            + f"  sym_data entries: {len(sym_data)} | candle counts: {[len(c) for _,c,*_ in sym_data[:5]]}"
        )
        return
    ...
    if args.leverage != 30:  # 30 is the default
        print(
            Fore.YELLOW
            + f"  [NOTE] --leverage {args.leverage}x specified, but it will be overridden by"
        )
    ...
    if args.sweep:
        ...
        sweep_res = sweep(
            valid, trail_pcts, sl_pcts, tp_pcts, min_scores, leverages, **sweep_kwargs
        )
                ...
                [
                    {
                        ...
                        "sl_pct": r.stop_loss_pct,
                        "tp_pct": r.take_profit_pct,
                        ...
                        "profit_factor": (
                            r.profit_factor if r.profit_factor < 9999 else 9999
                        ),
        ...
        label = (
            f"Trail {args.trail_pct*100:.1f}% | Score ≥{args.min_score} "
