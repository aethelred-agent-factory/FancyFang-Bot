...
class SectorManager:
    ...
    def __init__(self, window_size: int = 20):
        self.sector_momentum: Dict[str, deque] = {
            s: deque(maxlen=window_size) for s in set(SECTOR_MAP.values())
        }
    ...
    def get_all_sector_scores(self) -> Dict[str, float]:
        with self._lock:
            return {
                s: (sum(q) / len(q) if q else 0.0)
                for s, q in self.sector_momentum.items()
            }
