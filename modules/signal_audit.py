...
def run_audit(limit: int = 500):
    ...
    if not rows:
        print(
            "No raw signal data found in trade history. (New feature, needs fresh trades)"
        )
        return
    ...
    if len(filtered_rows) < 5:
        print(
            f"Insufficient complete data points (found {len(filtered_rows)}, need at least 5)."
        )
        return
    ...
    high_corr = pairs[
        (pairs["Signal 1"] < pairs["Signal 2"]) & (pairs["Correlation"].abs() > 0.70)
    ]

    if high_corr.empty:
        print("No high redundancies detected. Your signals are well-orthogonalized!")
    else:
        for _, row in high_corr.sort_values(
            "Correlation", key=abs, ascending=False
        ).iterrows():
            print(
                f"  [!] {row['Signal 1']:<15} <-> {row['Signal 2']:<15} | Correlation: {row['Correlation']:+.2f}"
            )

        print("\nRecommendations:")
        print(
            " - For high positive correlation: Consider removing one or combining them into a single weight."
        )
        print(
            " - For high negative correlation: They may be measuring opposite sides of the same phenomenon."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=500, help="Number of recent trades to analyze"
    )
